...
XmlRpcStruct make_failure(const std::string &message,
...
bool RemoteServer::start() {
  ...
  server_ = std::make_unique<XmlRpcServer>(
      port_,
...
std::string RemoteServer::build_keywords_page() const {
  ...
  out << "<meta name=\"viewport\" content=\"width=device-width, "
         "initial-scale=1\">";
  out << "<title>"
      << html_escape(library_info_.name.empty() ? "Robot Remote Server"
                                                : library_info_.name)
      << "</title>";
  out << "<style>body{font-family:system-ui,-apple-system,Segoe "
         "UI,Roboto,sans-serif;max-width:900px;margin:2rem auto;padding:0 "
         "1rem;line-height:1.5;}";
  out << "h1,h2{margin-bottom:.4rem;} .kw{border:1px solid "
         "#d7dbe0;border-radius:8px;padding:1rem;margin:1rem 0;}";
  out << "ul{margin:.4rem 0 0 1.2rem;}code{background:#f4f6f8;padding:.1rem "
         ".35rem;border-radius:4px;}";
  out << "details{margin:.6rem 0;}summary{cursor:pointer;font-weight:600;}";
  out << ".doc{white-space:pre-line;margin:.5rem 0 0 0;}</style></head><body>";

  out << "<h1>"
      << html_escape(library_info_.name.empty() ? "Robot Remote Library"
                                                : library_info_.name)
      << "</h1>";
  if (!library_info_.version.empty()) {
    out << "<p><strong>Version:</strong> " << html_escape(library_info_.version)
        << "</p>";
  }
  if (!library_info_.documentation.empty()) {
    out << "<details><summary>Library documentation</summary><div "
           "class=\"doc\">"
        << html_escape(library_info_.documentation) << "</div></details>";
  }
  ...
  for (const auto &pair : keywords_) {
    ...
    out << "<details><summary>Keyword documentation</summary><div "
           "class=\"doc\">"
        << html_escape(keyword.documentation.empty() ? "(no description)"
...
XmlRpcValue RemoteServer::handle_call(const std::string &method,
                                      const std::vector<XmlRpcValue> &params) {
  ...
  if (method == "run_keyword") {
    if (params.size() < 2) {
      return XmlRpcValue(
          make_failure("run_keyword expects at least 2 params", ""));
    }
    ...
    if (params.size() >= 3 &&
        std::holds_alternative<XmlRpcStruct>(params[2].value)) {
